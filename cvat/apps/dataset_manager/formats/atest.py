...
from cvat.apps.engine.models import FrameQuality, JobType
...
def create_frame_manifest_xml(frame_id: int, frame_dir: str, frame_files: list, visualization_files: list, file_fingerprints: list, frame_annotation) -> str:
    """
    Create manifest XML file for a single frame directory.
    
    Args:
        frame_id: Frame ID
        frame_dir: Path to frame directory
        frame_files: List of original image filenames
        visualization_files: List of visualization files (masks, overlays, comparisons)
        file_fingerprints: List of tuples (filename, fingerprint) for each frame file
        frame_annotation: Frame annotation object
    ...
    directory_name_elem = doc.createElement('directory_name')
    directory_name_elem.appendChild(doc.createTextNode(osp.basename(frame_dir)))
    manifest.appendChild(directory_name_elem)
    ...
    # Frame information directly under manifest
    frame_id_elem = doc.createElement('frame_id')
    frame_id_elem.appendChild(doc.createTextNode(str(frame_id)))
    manifest.appendChild(frame_id_elem)
    
    frame_name_elem = doc.createElement('frame_name')
    frame_name_elem.appendChild(doc.createTextNode(frame_annotation.name))
    manifest.appendChild(frame_name_elem)
    
    annotations_count_elem = doc.createElement('annotations_count')
    annotations_count_elem.appendChild(doc.createTextNode(str(len(frame_annotation.labeled_shapes))))
    manifest.appendChild(annotations_count_elem)
    ...
    for filename in visualization_files:
        if '_mask.png' in filename:
            file_type = 'mask_visualization'
        elif '_overlay.jpg' in filename:
            file_type = 'overlay_visualization'
        elif '_comparison.jpg' in filename:
            file_type = 'comparison_visualization'
        else:
            file_type = 'visualization'
...
def create_frame_metrics_xml(frame_id: int, frame_dir: str, labels_dict: dict, relative_distribution: dict, frame_annotation, file_fingerprint: str) -> str:
    """
    Create metrics XML file for a single frame directory.
    
    Args:
        frame_id: Frame ID
        frame_dir: Path to frame directory  
        labels_dict: Dictionary mapping label names to colors
        relative_distribution: Material distribution percentages for this frame
        frame_annotation: Frame annotation object
        file_fingerprint: SHA256 fingerprint of original image
    ...
    frame_metadata = doc.createElement('frame_metadata')
    metrics.appendChild(frame_metadata)
    
    frame_id_elem = doc.createElement('frame_id')
    frame_id_elem.appendChild(doc.createTextNode(str(frame_id)))
    frame_metadata.appendChild(frame_id_elem)
    
    frame_name_elem = doc.createElement('frame_name')
    frame_name_elem.appendChild(doc.createTextNode(frame_annotation.name))
    frame_metadata.appendChild(frame_name_elem)
    
    dimensions_elem = doc.createElement('dimensions')
    dimensions_elem.setAttribute('width', str(frame_annotation.width))
    dimensions_elem.setAttribute('height', str(frame_annotation.height))
    frame_metadata.appendChild(dimensions_elem)
    
    annotations_count_elem = doc.createElement('annotations_count')
    annotations_count_elem.appendChild(doc.createTextNode(str(len(frame_annotation.labeled_shapes))))
    frame_metadata.appendChild(annotations_count_elem)
    
    # File fingerprint section
    fingerprint_elem = doc.createElement('file_fingerprint')
    fingerprint_elem.setAttribute('algorithm', 'sha256')
    fingerprint_elem.setAttribute('source', 'original_uncompressed')
    fingerprint_elem.appendChild(doc.createTextNode(file_fingerprint))
    metrics.appendChild(fingerprint_elem)
    ...
    if relative_distribution:
        ...
        note_elem.appendChild(doc.createTextNode('No material distribution available for this frame'))
...
def _export_task_or_job(dst_file, temp_dir, instance_data, anno_callback, save_images=False):
    """
    Export function that creates individual ZIP files for each frame with all associated files.
    Creates per-frame ZIP packages containing original image, mask, overlay, comparison, manifest and metrics.
    """
    import zipfile
    import shutil
    ...
    # Collect all frame annotations ONCE to avoid consuming streaming generators
    all_frame_annotations = list(instance_data.group_by_frame(include_empty=True))
    ...
    if save_images:
        ...
        # Build lookup from frame index to pre-collected frame annotation
        frame_anno_lookup = {fa.idx: fa for fa in all_frame_annotations}
        ...
        for frame_id, frame in zip(instance_data.rel_range, frames):
            ...
            frame_annotation = frame_anno_lookup.get(frame_id)
            ...
            # Create individual frame directory
            ...
            img_path = osp.join(frame_dir, frame_name_jpg)
            ...
            # Save mask PNG (pure annotation data, no legend)
            ...
            # Save overlay as JPEG with compression
            ...
            # Create individual frame annotations.xml
            with open(osp.join(frame_dir, "annotations.xml"), "wb") as f:
                dump_task_or_job_anno(f, instance_data, anno_callback, frame_annotation)
            
            # Create frame-level manifest.xml
            frame_files = [frame_name_jpg]
            visualization_files = [mask_filename, overlay_filename, comparison_filename]
            file_fingerprints = [(frame_name_jpg, file_fingerprint)]  # Use compressed filename as key
            
            create_frame_manifest_xml(frame_id, frame_dir, frame_files, visualization_files, file_fingerprints, frame_annotation)
            
            # Create frame-level metrics.xml with distribution data
            create_frame_metrics_xml(frame_id, frame_dir, labels_dict, relative_distribution, frame_annotation, file_fingerprint)
            
            # Create ZIP file for this frame
            frame_zip_filename = f"{frame_name_base}.zip"
            frame_zip_path = osp.join(temp_dir, frame_zip_filename)
            
            # Create the frame ZIP file
            with zipfile.ZipFile(frame_zip_path, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as frame_zip:
                # Walk through the frame directory and add all files to ZIP
                for root, dirs, files in os.walk(frame_dir):
                    for file in files:
                        file_path = osp.join(root, file)
                        # Create relative path within the ZIP (remove the frame_dir prefix)
                        arcname = osp.relpath(file_path, frame_dir)
                        frame_zip.write(file_path, arcname)
            
            # Remove the original frame directory after creating ZIP
            shutil.rmtree(frame_dir)
    
    else:
        # If save_images=False, create a single annotations file
        with open(osp.join(temp_dir, "annotations.xml"), "wb") as f:
            dump_task_or_job_anno(f, instance_data, anno_callback, None)
...
@exporter(name="CVAT Custom Video Old", ext="ZIP", version="1.1")
...
@exporter(name="CVAT Custom NES Format Old", ext="ZIP", version="1.1")
...
@importer(name="CVAT Custom Old", ext="XML, ZIP", version="1.1")
