...
def dump_job_annotations(dst_file, job_data, anno_callback, frame_annotations=None):
    """
    ...
    if frame_annotations is None:
        frame_annotations = job_data.group_by_frame(include_empty=True)
...
def _export_task_or_job(dst_file, temp_dir, instance_data, anno_callback, save_images=False):
    ...
    for job_info in get_jobs_for_instance(instance_data):
        ...
        # (AnnotationIR may use generators that can only be iterated once)
        all_frame_annotations = list(job_data.group_by_frame(include_empty=True))
        ...
        with open(osp.join(job_dir, "annotations.xml"), "wb") as f:
            dump_job_annotations(f, job_data, anno_callback, frame_annotations=all_frame_annotations)
        ...
        if save_images:
            ...
            for frame_id, frame in zip(job_data.rel_range, frames):
                if frame_id not in included_frames:
                    continue
